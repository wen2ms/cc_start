...
void print_students(const Student students[], int num_students) {
    std::cout << std::left << std::setw(15) << "Name" << std::setw(10) << "Score" << std::setw(5) << "Age" << '\n';
    std::cout << std::left << std::string(30, '*') << '\n';

    for (int i = 0; i < num_students; i++) {
        std::cout << std::left << std::setw(15) << students[i].name << std::setw(10) << students[i].score << std::setw(5)
                  << students[i].age << '\n';
    }
}
