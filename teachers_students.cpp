...
void display_title(void) {
    std::cout << std::left << std::setw(kNameWidth) << "Teacher Name" << std::setw(kIDWidth) << "ID" << std::setw(kNameWidth)
              << "Student Name" << std::setw(kTotalScore) << "Total Score\n";
...
void display_teacher(const Teacher& teacher) {
    for (const Student& student : teacher.students) {
        std::cout << std::left << std::setw(kNameWidth) << teacher.name << std::setw(kIDWidth) << teacher.ID
                  << std::setw(kNameWidth) << student.name << std::setw(kTotalScore) << student.total_score << std::endl;
    }
}

int main(void) {
    Teacher teachers[3] = {{"Mr. Smith", 101, {{"Peter", 85}, {"Bob", 90}, {"Charlie", 92}}},
